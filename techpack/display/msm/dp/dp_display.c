...
static int dp_display_process_hpd_high(struct dp_display_private *dp)
{
	...
	if (rc) {
		...
		DP_INFO("fall through fail safe %d\n", rc);
		if (rc == -EINVAL)
			edid_fail = true;
		is_poor_connection = true;
		goto notify;
		...
		CC_NOTI_TYPEDEF *noti, bool connect)
{
	...
	dp->sec.dex.res = connect ?
		dp->sec.adapter.dex_type : DEX_RES_NOT_SUPPORT;
...
int secdp_wait_for_disconnect_complete(void)
{
	...
	if (dp->is_dp_disconnecting) {
		...
		if (ret > 0) {
			DP_INFO("dp_disconnect complete!\n");
			ret = 0;
		} else {
			DP_ERR("dp_disconnect timeout\n");
			ret = -1;
		}
	}
				...
				struct drm_display_mode *mode, bool supported)
{
	...
	if (!supported || mode->vrefresh < MIRROR_REFRESH_MIN)
		goto end;
	...
	struct dp_display_mode *dp_mode)
{
	...
	DP_INFO("max mirror:%d, max dex:%d, ignore_ratio:%d, ret:%d\n",
