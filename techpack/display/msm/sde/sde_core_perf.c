		...
		struct sde_core_perf_params *perf)
{
	...
	SDE_EVT32(DRMID(crtc), perf->core_clk_rate,sde_cstate->bw_control,
			 kms->perf.max_core_clk_rate,kms->perf.perf_tune.mode);
		...
		int params_changed, bool stop_req)
{
	...
	if (update_clk) {
...
#if defined(CONFIG_DISPLAY_SAMSUNG)
		{
			...
			if (flag_boost_mdpclk_cwb) {
				SDE_DEBUG("During CWB: keep max SDE core clock (%lld -> %lld hz)\n",
						clk_rate, kms->perf.max_core_clk_rate);
				clk_rate = kms->perf.max_core_clk_rate;
			}

			if (flag_boost_mdpclk_screenrecorder) {
				SDE_DEBUG("During screenrecorder: keep max SDE core clock (%lld -> %lld hz)\n",
						clk_rate, kms->perf.max_core_clk_rate);
				clk_rate = kms->perf.max_core_clk_rate;
			}
