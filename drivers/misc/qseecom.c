	...
	case SCM_SVC_TZSCHEDULER: {
		...
		case QSEOS_GENERATE_KEY: {
			...
			if (!tzbuf)
				return -ENOMEM;
			memset(tzbuf, 0, tzbuflen);
			memcpy_noio(tzbuf, req_buf + sizeof(uint32_t),
		...
		case QSEOS_UPDATE_KEY_USERINFO: {
			...
			if (!tzbuf)
				return -ENOMEM;
			memset(tzbuf, 0, tzbuflen);
			memcpy_noio(tzbuf, req_buf + sizeof(uint32_t), (sizeof
				(struct qseecom_key_userinfo_update_ireq) -
				sizeof(uint32_t)));
			dmac_flush_range(tzbuf, tzbuf + tzbuflen);
				...
				uint32_t app_id)
{
	...
	switch (resp.result) {
	case QSEOS_RESULT_SUCCESS:
		pr_warn("App %d (%s) is unloaded\n", app_id, data->client.app_name);
		break;
	case QSEOS_RESULT_INCOMPLETE:
		ret = __qseecom_process_incomplete_cmd(data, &resp);
		if (ret)
